...
**Final Step: Handoff Save.** Run `mkdir -p .claude/crew-state/marketing`, then write `.claude/crew-state/marketing/crew-marketing-landing-page-review-handoff.md` with: the review and score produced, decisions made (the weakest element chosen, the CTA rewrite), unfinished work (fields marked "Not provided", anything escalated), what `crew-marketing-seo-page-builder` or `crew-marketing-brand-voice-check` needs next, and any "Learned" note (a correction or preference the user gave, for example the real conversion goal). Always write it, even with no output ("No output, run completed [date]"). (Loop 4 and Loop 5.)
