...
**Brand carrier:** if the client supplied a brand kit or a live site exists, consult `crew-design-language` FIRST (with the CREW CONSULT preamble) and map its extracted tokens into the carrier; a kit already shaped by `crew-design-kit` maps directly. Otherwise adapt the minimal-luxe tokens to the product's palette (reference build: warm ink #0a0805, warm ivory, stone, chestnut-brass accent). One accent, keyed to the arrival and em words only. Inter 100-300 with letterspaced uppercase labels unless the client's kit says otherwise.
...
9. **Review gate.** Run `crew-design-quality` (binding) with `crew-design-composition`, `crew-design-patterns`, and `crew-design-engineering` on the built file + live local URL (see Design review gate). Fix all Criticals and Majors; a Fail or unaddressed Revise is Loop 2 until it passes. In Governed mode nothing is waived.
...
- `crew-design-language` (pack 12): consult FIRST when the client supplies a brand kit or a live site exists; map its extracted tokens into the carrier before adapting the minimal-luxe defaults (Step 7).
- `crew-design-quality` (binding) + `crew-design-composition` + `crew-design-patterns` + `crew-design-engineering`: the pre-deploy gate (Step 9).
