...
- **The slide stays fully visible while the sidebar is open.** The canvas reflows beside the panel (a margin shift or a proportional scale, transitioned), never sliding under it: the facilitator edits what they can see, and every block remains clickable. The sidebar itself collapses and reopens from a visible affordance without dropping edit mode, so a full-width look at the slide is one click away, not a mode exit.
...
[ ] Edit mode: E (or the visible affordance) opens the sidebar (modules -> steps
    -> blocks); the canvas reflows beside the open sidebar with every block still
    visible and clickable, never covered, and the sidebar collapses and reopens
    without leaving edit mode; blocks edit inline; media blocks accept a pasted URL; steps add,
