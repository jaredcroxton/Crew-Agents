...
EXPECT:
- Output begins with "LEAD DASHBOARD BUILD REPORT" and a summary line with Leads, Hot, Warm, Cool, Decision-makers, Emails, DMs.
...
- Each decision-maker card has a clickable LinkedIn link that opens in a new tab (a direct profile URL, or a LinkedIn people-search link where no direct profile is public). No profile URL is fabricated.
...
- Fit scores still computed (Derived) with sub-scores, so even thin leads are ranked.
- Handoff file written, listing the Derived contacts and escalated insights to verify.
...
- It runs the five discovery questions one at a time rather than guessing.
...
- Handoff file written, recording the build as not started and the inputs still needed.
