...
**Final Step: Handoff Save.** Run `mkdir -p .claude/crew-state/training`, then write `.claude/crew-state/training/crew-training-needs-analyser-handoff.md` with: the gap report produced, decisions made (the ranking and its basis), unfinished work (gaps marked "Assumed" or impact unquantified), anything escalated (motivation or process gaps routed out), what `crew-training-module-outline-builder` needs next (the top topic and its level), and any "Learned" note (a correction or business fact the manager gave). Always write it, even with no output ("No output, run completed [date]"). (Loop 4 and Loop 5.)
