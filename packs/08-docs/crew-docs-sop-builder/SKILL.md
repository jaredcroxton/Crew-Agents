...
**Final Step: Handoff Save.** Run `mkdir -p .claude/crew-state/docs`, then write `.claude/crew-state/docs/crew-docs-sop-builder-handoff.md` with: the SOP produced, decisions made (granularity chosen, where approvals were placed), unfinished work (steps with "not provided" owners, anything escalated), what `crew-docs-training-guide-creator` needs next to teach this SOP, and any "Learned" note (a correction or preference the owner gave, for example a role title or an approval rule). Always write it, even with no output ("No output, run completed [date]"). (Loop 4 and Loop 5.)
