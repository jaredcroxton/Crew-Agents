...
**Final Step: Handoff Save.** Run `mkdir -p .claude/crew-state/docs`, then write `.claude/crew-state/docs/crew-docs-client-playbook-builder-handoff.md` with: the playbook produced, decisions made (type chosen, sections included or dropped), unfinished work (fields marked to confirm, anything escalated), what `crew-docs-sop-builder` needs to write the matching internal version, and any "Learned" note (a correction or preference the user gave). Always write it, even with no output ("No output, run completed [date]"). (Loop 4 and Loop 5.)
...
[ ] The handoff was written to .claude/crew-state/docs/
