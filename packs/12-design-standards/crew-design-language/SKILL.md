...
- **Governed mode:** the full system, plus a cross-reference against prior records in this project (`~/.claude/crew-state/projects/<project>/`) so the language carries across every page and build, the project brand playbook enforced as the primitive source of truth, and the full drift audit. Use for a multi-page product or a system handed to a team.
