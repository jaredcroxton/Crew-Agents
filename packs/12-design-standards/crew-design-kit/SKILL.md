...
- **Governed mode:** the full kit, plus a cross-reference against prior handoffs in `~/.claude/crew-state/design-standards/` so one kit carries across a whole project (the page, the deck, the dashboard all share it), AA enforced and AAA checked for body text, and the chosen kit recorded so later skills inherit it. Use when the kit feeds a client deliverable.
