...
The play library: the proven routines for running a business on the Crew. A play is one runnable move: when to use it, the exact prompt to say, what comes back, and the next move. A chain play is several skills in one project, each step feeding the next through the handoff records.

How to use this file (dispatcher instructions): when the user asks "what play do I run", "what should I use for X", "how do I do X end to end", or their words match a play's intents line, find the matching play here and return its card whole (When, Prompt, You get, Next, Tip), then offer to run the prompt now. When a request spans several skills, offer the matching chain play and run it step by step inside one project. The intents lines are the match index: plain words a user actually says. Prompts assume the business is onboarded; if not, route to onboarding first (the first play).
...
### Turn a lead list into a ranked call sheet

- **When:** You have a list of companies (a scrape target, a directory, an expo list) and need to know who to call first and what to say.
- **Prompt:** `New project, call it leads-[niche]. Run crew-web-lead-dashboard-builder on [source or list]: fit-score every lead 0 to 100, find the decision-maker, and draft the first touch for each.`
- **You get:** A branded one-page dashboard: every lead scored and ranked, the decision-maker with backups, a cold email and a LinkedIn DM per lead, all evidence-tagged with verify-before-send flags.
- **Next:** Work top to bottom: call or run the outreach play on the highest scores first.
- **Tip:** It renders as a web page but it is a sales tool: open it before a call block and treat it as the day's call sheet.
- **Intents:** lead dashboard, lead list, score leads, who do I call, prospect list, rank leads, call sheet, scrape leads
...
### The PowerPoint killer: present it as an online journey (Showcase pack)

- **When:** Delivery day: replace the deck with a presented journey the room follows. Needs `crew-web-learning-experience` from the Showcase add-on pack; if it is not installed, say so instead of running the prompt.
...
Several plays, one project, each step feeding the next. Run them in order; every step reads the records the step before it wrote. Steps marked Showcase pack need the Showcase add-on installed; if it is absent, say so and deliver the chain up to that step.

### Chain 01: Learning design, end to end

1. `crew-training-needs-analyser`
2. `crew-training-module-outline-builder`
3. `crew-training-facilitator-guide-creator`
4. `crew-training-learner-workbook-builder`
5. `crew-training-assessment-designer`
6. `crew-web-learning-experience` (Showcase pack)
...
1. `crew-sales-lead-research`
2. `crew-sales-outreach-draft`
3. `crew-web-slide-deck-mobile` (Mobile Quote)
4. `crew-finance-invoice-workflow`
...
1. `crew-marketing-campaign-plan`
2. `crew-marketing-social-post-pack`
3. `crew-marketing-email-campaign-builder`
4. `crew-marketing-landing-page-review`

**Note:** Plan once; every asset inherits the angle inside the same project.

### Chain 04: New hire, day zero to day 90

1. `crew-hr-role-profile-builder`
2. `crew-hr-interview-guide`
3. `crew-training-onboarding-programme-builder`
4. `crew-training-coaching-conversation-guide`

**Note:** The role's outcomes flow through interviews into onboarding automatically.

### Chain 05: Research to deliverable

1. `crew-docs-research-notebooklm`
2. `crew-sales-proposal-builder` or `crew-web-slide-deck-builder`
3. `crew-core-quality-checker`

**Note:** Grounded, cited research feeds whatever you build; the gate checks the claims survived.

### Chain 06: Knowledge capture

1. `crew-ops-process-map`
2. `crew-docs-sop-builder`
3. `crew-docs-training-guide-creator`
4. `crew-web-learning-experience` (Showcase pack)
...
1. `crew-finance-monthly-summary`
2. `crew-finance-cashflow-brief`
3. `crew-finance-expense-review`
4. `crew-finance-invoice-workflow`
...
1. `crew-support-ticket-triage`
2. `crew-support-feedback-summary`
3. `crew-support-faq-builder`
4. `crew-ops-workflow-improvement`
...
1. `crew-marketing-brand-voice-check`
2. `crew-marketing-content-repurpose`
3. `crew-marketing-social-post-pack`
4. `crew-marketing-email-campaign-builder`

**Note:** One good piece, voice-checked, becomes a fortnight of content across every channel.

### Chain 10: Website launch

1. `crew-web-page-builder`
2. `crew-marketing-seo-page-builder`
3. `crew-marketing-landing-page-review`
4. `crew-core-quality-checker`

**Note:** Build it, make it findable, make it convert, gate it. Then deploy.

### Chain 11: Client delivery, start to finish

1. `crew-sales-prospect-brief`
2. `crew-sales-proposal-builder`
3. `crew-docs-handover-document-writer`
4. `crew-docs-client-playbook-builder`
...
1. `crew-hr-performance-conversation-prep`
2. `crew-hr-employee-communication-draft`
3. `crew-training-coaching-conversation-guide`
4. `crew-training-skill-gap-mapper`
