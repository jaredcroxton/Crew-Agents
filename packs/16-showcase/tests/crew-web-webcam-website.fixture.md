...
- The Design review gate is run: `crew-design-quality` (binding), `crew-design-engineering` (binding, the pixel-and-easing leg), `crew-design-reference` (composition lens), `crew-design-reference` (patterns lens), a register-conditional pack-13 style lens (here `crew-design-styles` (soft lens) for the warm premium register), with `crew-animation` (css spec) / `crew-animation` (scroll-reveal spec) / `crew-animation` (view-transitions spec) (and gsap / motion for the scrub feel) consulted as authoring references only, Criticals and Majors fixed, a fail blocking the ship. Each leg is invoked with the `CREW CONSULT from crew-web-webcam-website:` preamble.
