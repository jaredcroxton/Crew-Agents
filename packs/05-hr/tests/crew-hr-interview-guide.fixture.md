...
- Four to six capabilities are tested, each typed and drawn from the profile (conflicting priorities as Behavioural, the forecast as Technical, cold-chain compliance as a Technical role-specific knowledge check, "owns the problem" as Values-based), none invented; "role-specific" is not a separate question type.
...
- No question touches a protected characteristic or a proxy (no age, family, origin, employment gap, salary-history, graduation-date).
